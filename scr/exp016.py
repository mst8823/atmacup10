...
from scr.mypipe.features.text_engine import BasicTextFeatureTransformerEngine, TextVectorizer
...
config = Config(EXP_NAME="exp016", TARGET="likes")
...
@cach_feature(feature_name="weighted_technique_features", directory=config.FEATURE)
def get_weighted_technique_features(input_df):
    _tec = technique.copy()
    _df = pd.merge(input_df[["object_id", "likes"]], _tec, on="object_id", how="left")

    mapping = np.log1p(_df.groupby("name")["likes"].mean())
    _tec["wname"] = _tec["name"].map(mapping)
    _df = pd.pivot_table(_tec, index="object_id", columns="name", values="wname").fillna(0)
    ...
    pca_df = get_pca(_df, 8, "like_weighted_technique_PCA")
    pca_df["object_id"] = _df.index
    output_df = pd.merge(input_df[["object_id"]], pca_df, on="object_id", how="left").drop("object_id", axis=1)
    return output_df
...
@cach_feature(feature_name="language", directory=config.FEATURE)
def get_language(input_df):
    output_df = pd.DataFrame()
    tmp = input_df["title"] + " " + input_df["long_title"] + " " + input_df["description"].fillna("") + " " + input_df[
        "more_title"].fillna("")
    output_df["language"] = [detect(x) for x in tqdm(tmp)]
    return output_df
...
@cach_feature(feature_name="agg_nunique_language_features", directory=config.FEATURE)
def get_agg_nunique_language_features(input_df):
    _input_df = pd.concat([input_df, get_language(input_df)], axis=1)
    group_key = "language"
    group_values = [
        "art_series_id",
        "title",
...
def get_basic_text_features(input_df):
    ...
    _input_df["concat_title"] = _input_df["title"] + " " + _input_df["long_title"] + " " + _input_df[
        "more_title"].fillna("")
    encoder = BasicTextFeatureTransformerEngine(text_columns=["title", "long_title", "more_title", "description", "principal_maker", "concat_title"])
    output_df = encoder.fit_transform(_input_df)
    return output_df
...
def get_text_vec_features(input_df):
    ...
    _input_df["concat_title"] = _input_df["title"] + " " + _input_df["long_title"] + " " + _input_df[
        "more_title"].fillna("")
    encoder = TextVectorizer(text_columns=["title", "long_title", "more_title", "description", "concat_title", "principal_maker"],
...
def merge_data(input_df):
    funcs = [
        ...
        # get_weighted_technique_features,
...
def preprocess(train, test):
    ...
    process_blocks = [
        ...
        get_agg_nunique_language_features,
